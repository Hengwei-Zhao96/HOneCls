...
class ScalarRecorder:
    ...
    def update_gradient(self, scalar):
    ...
    def save_kde_fig(self, saved_path):
        ...
        sns.kdeplot(scalar)
        plt.savefig(os.path.join(saved_path, 'mean_' + str(scalar_mean) + '_std_' + str(scalar_std) + '.png'))

    def save_lineplot_fig(self, y_name, fig_name, saved_path):
        index_scalar = zip(list(range(1, len(self.scalar))), np.asarray(self.scalar))
